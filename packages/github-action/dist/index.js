...
var require_simple_swizzle = __commonJS((exports2, module2) => {
...
var require_color_string = __commonJS((exports2, module2) => {
  ...
  function _pipeline(callback, stack) {
    if (typeof callback === "function") {
      if (this._isStreamInput()) {
        this.on("finish", () => {
          this._flattenBufferIn();
          sharp.pipeline(this.options, (err, data, info2) => {
            if (err) {
              callback(is.nativeError(err, stack));
            } else {
...
function setOutput(name, value) {
  ...
  if (outputPath) {
    import_node_fs.appendFileSync(outputPath, `${name}=${value}
`, "utf8");
    return;
  }
...
// ../core/src/core/compression-engine.ts
...
// ../core/src/services/image-compressor.service.ts
var import_sharp = __toESM(require_lib());
var import_node_path2 = require("node:path");

// ../core/src/utils/file-size.ts
var import_promises = require("node:fs/promises");
function targetSizeToBytes(targetSize) {
...
async function getFileSizeBytes(filePath) {
  const fileStats = await import_promises.stat(filePath);
...
function buildDefaultOutputPath(inputPath) {
  const parsed = import_node_path.default.parse(inputPath);
  return import_node_path.default.join(parsed.dir, `${parsed.name}.squished${parsed.ext}`);
}
function buildOutputPathInDir(inputPath, outputDir) {
  return import_node_path.default.join(outputDir, import_node_path.default.basename(inputPath));
}
...
function resolveOutputPath(job) {
  ...
  const parsed = import_node_path2.parse(fallback);
  if (parsed.ext.toLowerCase() === ".webp") {
    return fallback;
  }
  return import_node_path2.join(parsed.dir, `${parsed.name}.webp`);
}
async function compressImage(job) {
  ...
  try {
    ...
    let pipeline = import_sharp.default(job.inputPath, { failOn: "none" });
    if (job.preset.resolutionScale && job.preset.resolutionScale < 1) {
      ...
      if (metadata.width || metadata.height) {
        ...
        const height = metadata.height ? Math.max(1, Math.round(metadata.height * job.preset.resolutionScale)) : undefined;
        pipeline = pipeline.resize({ width, height, fit: "inside", withoutEnlargement: true });
      }
    }
    const inputExtension = import_node_path2.parse(job.inputPath).ext.toLowerCase();
    ...
    const reductionPercent = originalSizeBytes ? Number(((originalSizeBytes - compressedSizeBytes) / originalSizeBytes * 100).toFixed(2)) : undefined;
    return {
    ...
    return {
      ...
      outputPath,
      success: false,
      error: error2 instanceof Error ? error2.message : "Unknown image compression error"
    };
  }
}

// ../core/src/services/pdf-compressor.service.ts
var import_node_child_process = require("node:child_process");
var GHOSTSCRIPT_PROFILE_MAP = {
  low: "/prepress",
  medium: "/printer",
  high: "/ebook",
  extreme: "/screen"
...
function runGhostscript(inputPath, outputPath, profile) {
  return new Promise((resolve, reject) => {
    ...
    const child = import_node_child_process.spawn("gs", args, { stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
...
async function compressPdf(job) {
  ...
  try {
    ...
    return {
      ...
      outputPath,
      success: false,
      error: error2 instanceof Error ? `Ghostscript execution error: ${error2.message}` : "Unknown PDF compression error"
    };
  }
}

// ../core/src/utils/file-detect.ts
var import_node_path3 = require("node:path");
var IMAGE_EXTENSIONS = new Set([".jpg", ".jpeg", ".png", ".webp"]);
var PDF_EXTENSIONS = new Set([".pdf"]);
var IMAGE_MIME_TYPES = new Set(["image/jpeg", "image/jpg", "image/png", "image/webp"]);
var PDF_MIME_TYPES = new Set(["application/pdf"]);
function detectSupportedFileType(filePath, mimeType) {
  const extension = import_node_path3.extname(filePath).toLowerCase();
...
// ../core/src/core/compression-engine.ts
var MAX_TARGET_ATTEMPTS = 6;
var QUALITY_FLOOR = 25;
async function runCompressionEngine(inputPaths, preset, options = {}) {
...
async function compressToTargetSize(job) {
  const tempDir = await import_promises2.mkdtemp(import_node_path4.join(import_node_os.tmpdir(), "squish-target-"));
  ...
  try {
    while (attempts < MAX_TARGET_ATTEMPTS && low <= high) {
      ...
      const parsed = import_node_path4.parse(job.inputPath);
      const attemptOutput = import_node_path4.join(tempDir, `${parsed.name}.q${quality}${parsed.ext || ".tmp"}`);
      const attemptJob = {
        ...job,
    ...
    if (finalOutputPath !== bestResult.outputPath) {
      await import_promises2.rename(bestResult.outputPath, finalOutputPath);
    }
    return {
      ...bestResult,
    ...
    await import_promises2.rm(tempDir, { recursive: true, force: true });
  }
}
async function buildCompressionJobs(inputPaths, preset, outputDir) {
  const jobs = [];
  for (const inputPath of inputPaths) {
    const discoveredFiles = await collectFiles(import_node_path4.resolve(inputPath));
...
async function collectFiles(inputPath) {
  try {
    const stats = await import_promises2.stat(inputPath);
    if (stats.isFile()) {
      return [inputPath];
    }
    ...
    const entries = await import_promises2.readdir(inputPath, { withFileTypes: true });
    const nestedFiles = await Promise.all(entries.map((entry) => collectFiles(import_node_path4.join(inputPath, entry.name))));
...
function parseInputs() {
  const presetInput = getInput("preset", { required: true }).trim().toLowerCase();
  if (!(presetInput in PRESETS)) {
    throw new Error(`Invalid preset "${presetInput}". Allowed: ${Object.keys(PRESETS).join(", ")}.`);
  }
  return {
    inputPath: getInput("path", { required: true }),
    preset: presetInput,
    targetSize: parseTargetSize(getInput("target-size", { required: true })),
    commit: getBooleanInput("commit", { required: true }),
    artifactPath: getInput("artifact", { required: true }).trim() || undefined,
    maxSize: parseTargetSize(getInput("max-size", { required: true })),
    dryRun: getBooleanInput("dry-run", { required: true }),
    includeGlobs: parseListInput(getInput("include-glob", { required: true })),
    excludeGlobs: parseListInput(getInput("exclude-glob", { required: true }))
  };
}
...
function globToRegex(pattern) {
  ...
  const tokenized = normalized.split("**").map((part) => part.split("*").map(escapeRegex).join("[^/]*")).join(".*");
...
function setOutputs(results, filesScanned, maxSizeBytes) {
  ...
  const budgetViolations = maxSizeBytes === undefined ? 0 : optimized.filter((result) => (result.compressedSizeBytes ?? 0) > maxSizeBytes).length;
  setOutput("files-scanned", filesScanned.toString());
  setOutput("files-optimized", optimized.length.toString());
  setOutput("bytes-before", bytesBefore.toString());
  setOutput("bytes-after", bytesAfter.toString());
  setOutput("bytes-saved", bytesSaved.toString());
  setOutput("savings-percent", savingsPercent);
  setOutput("budget-violations", budgetViolations.toString());
}
async function writeArtifact(artifactPath, payload) {
  await import_promises3.mkdir(import_node_path5.dirname(artifactPath), { recursive: true });
  await import_promises3.writeFile(artifactPath, JSON.stringify(payload, null, 2));
}
async function maybeCommitResults(results) {
  for (const result of results) {
    if (!result.success || result.outputPath === result.inputPath)
      continue;
    await import_promises3.cp(result.outputPath, result.inputPath, { force: true });
    await import_promises3.rm(result.outputPath, { force: true });
  }
}
async function run() {
  try {
    ...
    const resolvedPreset = inputs.targetSize === undefined ? preset : { ...preset, targetSizeBytes: targetSizeToBytes(inputs.targetSize) };
    const rootPath = import_node_path5.resolve(inputs.inputPath);
    ...
    info(`Discovered ${allJobs.length} supported files; ${filteredJobs.length} matched include/exclude filters.`);
    if (inputs.dryRun) {
      setOutputs([], filteredJobs.length, inputs.maxSize ? targetSizeToBytes(inputs.maxSize) : undefined);
      if (inputs.artifactPath) {
        await writeArtifact(import_node_path5.resolve(inputs.artifactPath), { mode: "dry-run", files: filteredJobs.map((job) => job.inputPath) });
      }
      return;
    }
    const results = await runCompressionEngine(filteredJobs.map((job) => job.inputPath), resolvedPreset, {
      onProgress: ({ completed, total, result }) => {
        info(`[${completed}/${total}] ${result.inputPath}: ${result.success ? "ok" : "failed"}`);
      }
    });
    if (inputs.commit) {
    ...
    if (inputs.artifactPath) {
      await writeArtifact(import_node_path5.resolve(inputs.artifactPath), {
    ...
    if (failures.length > 0) {
      failures.forEach((failure) => error(`${failure.inputPath}: ${failure.error ?? "unknown error"}`));
      setFailed(`${failures.length} compression job(s) failed.`);
    }
  } catch (error2) {
    setFailed(error2 instanceof Error ? error2.message : "Unexpected github action error.");
  }
}
run();
